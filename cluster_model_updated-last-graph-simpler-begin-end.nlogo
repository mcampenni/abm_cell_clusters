...
to create-env
  ...
  ;  ask n-of foods patches with [pcolor = black] [
  ;    set pcolor green
  ;    set food 1
  ;  ]
  new-food
...
to setup-cell-properties
  ;;;[surface anoikis mutation nlinks vulnerability labour energy metabolic]
  ask turtles[
...
to update-cell-properties
    ask turtles[
    set nlinks count turtles in-radius 1 with [self != myself]
    ;;initialize an agentset of links...
    set surface 1 / (nlinks + 1)
    set vulnerability 1 / (1 + (exp (-1 * (((surface * 2) - 1) * 4))))
    ;;set labour 0
    ]
...
to food-degradation
  ;;;update the availability of food depending of time
  ask patches [
...
;to regrow-food
;  ;;;decide between "seasonality" or "constant concentration" or....
;  ;; x / 1 + |x| sigmoid-like curve
;  if ticks > 0 [
;    if random-float 1 < (ticks / (1 + ticks)) * 2 - 1 [
;;    if (1 / 1 + exp(-1 * (ticks / 10))) - 1 < random-float 1 [
;      ask n-of 200 patches [
;        set food original-food
;      ]
;    ]
;  ]
;end
